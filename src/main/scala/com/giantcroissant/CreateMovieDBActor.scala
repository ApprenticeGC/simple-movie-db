...
class CreateMovieDBActor(implicit val mongoConnection: MongoConnection) extends Actor with ActorLogging {
  import scala.concurrent.ExecutionContext.Implicits.global
  def receive = {
    case CreateMovieActorInDBMessage(data) => {
      ...
      val furtureOperation = collection.insert(data)
      ...
      furtureOperation pipeTo sender
    }
  }
}