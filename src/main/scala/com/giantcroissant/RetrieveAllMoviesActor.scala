...
object RetrieveAllMoviesActor {
  case class RetrieveAllMoviesFromDBMessage()
  case class DoneRetrieveAllMoviesFromDBMessage(data: JsObject)
}

class RetrieveAllMoviesActor(implicit val mongoConnection: MongoConnection) extends Actor with ActorLogging {
  val dbActor = context.actorOf(Props(new RetrieveAllMoviesDBActor()))

  def receive = {
    case RetrieveAllMoviesRestMessage(result) => {
      log.info("RetrieveAllMoviesRestMessage")
      dbActor ! RetrieveAllMoviesFromDBMessage()
      context.become(waitSubActorDone)
    }
  }

  def waitSubActorDone: Receive = {
    case DoneRetrieveAllMoviesFromDBMessage(data) => {
      context.parent ! DoneRetrieveAllMoviesRestMessage()
    }
    case l:JsArray => {
      context.parent ! DoneRetrieveAllMoviesRestMessage(l)
    }
  }
}
